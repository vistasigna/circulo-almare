...
async function enviarEmail(para, assunto, html) {
  try { await mailer.sendMail({ from: `"ALMARE" <${process.env.SMTP_USER}>`, to: para, subject: assunto, html }); }
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-link { padding: 8px 16px; font-size: 11px; letter-spacing: 0.15em; text-transform: uppercase; color: var(--muted); border: 1px solid var(--border); border-radius: 3px; transition: all .2s; }
  .nav-link:hover, .nav-link.ativo { color: var(--gold); border-color: var(--gold); }
  .funcao-card { border: 1px solid var(--border); border-radius: 4px; padding: 16px 20px; cursor: pointer; transition: all .2s; display: flex; align-items: flex-start; gap: 14px; }
  .funcao-card:hover { border-color: var(--gold); }
  .funcao-card.selecionado { border-color: var(--gold); background: rgba(201,169,110,0.05); }
  .funcao-card.fixo { opacity: .6; cursor: default; }
  .funcao-card .check { width: 18px; height: 18px; border: 1px solid var(--border); border-radius: 3px; flex-shrink: 0; margin-top: 2px; display: flex; align-items: center; justify-content: center; font-size: 12px; color: var(--gold); }
  .funcao-card.selecionado .check, .funcao-card.fixo .check { background: rgba(201,169,110,0.2); border-color: var(--gold); }
  .funcao-card .funcao-nome { font-family: 'Cormorant Garamond', serif; font-size: 17px; color: var(--text); margin-bottom: 3px; }
  .funcao-card .funcao-desc { font-size: 12px; color: var(--muted); line-height: 1.5; }
  @media(max-width:600px) { .grid-2,.grid-3 { grid-template-columns: 1fr; } }
`;

function html(titulo, corpo, navLogado = false) {
  const nav = navLogado ? `
    <div style="display:flex;gap:12px;align-items:center;justify-content:flex-end;margin-top:12px;flex-wrap:wrap;">
  ...
  <body><div class="container"><header><div class="logo">ALMARE</div><div class="logo-sub">Círculo</div>${nav}</header>${corpo}</div></body></html>`;
}

// ── FUNÇÕES disponíveis para candidatura ────────────────────────────────────
const FUNCOES_DISPONIVEIS = [
  { slug: 'embaixador',    nome: 'Embaixador',    desc: 'Apresenta a ALMARE para outras pessoas e acompanha os resultados.' },
  { slug: 'especificador', nome: 'Especificador', desc: 'Arquiteto ou designer que incorpora obras ALMARE em projetos.' },
  { slug: 'anfitriao',     nome: 'Anfitrião',     desc: 'Recebe e apresenta experiências ALMARE para convidados.' },
  { slug: 'colaborador',   nome: 'Colaborador',   desc: 'Profissional que contribui para o ecossistema (fotografia, design, produção...).' },
  { slug: 'curador',       nome: 'Curador',       desc: 'Participa de decisões curatoriais e construção de coleções.' },
  { slug: 'artista',       nome: 'Artista',       desc: 'Submete obras originais para tiragem no catálogo ALMARE.' },
  { slug: 'guardiao',      nome: 'Guardião',      desc: 'Possui ou recebeu uma obra ou matriz especial da ALMARE.' },
];
...
app.get('/convite/:codigo', async (req, res) => {
  ...
  const funcaoCards = FUNCOES_DISPONIVEIS.map(f => `
    <label class="funcao-card" id="card-${f.slug}" onclick="toggleCard(event,'${f.slug}')">
      <div class="check" id="chk-${f.slug}"></div>
      <div>
        <div class="funcao-nome">${f.nome}</div>
        <div class="funcao-desc">${f.desc}</div>
      </div>
      <input type="checkbox" name="funcoes" value="${f.slug}" style="display:none" id="cb-${f.slug}">
    </label>`).join('');

  res.send(html('Faça parte', `
    <div class="container-sm" style="padding-top:10px;">
      <h1 style="font-size:34px;margin-bottom:8px;">Uma obra não é uma compra.</h1>
      <h1 style="font-size:34px;color:var(--gold);margin-bottom:28px;">É uma escolha de permanência.</h1>
      ...
      <hr class="divider">
      <h2 style="font-size:22px;margin-bottom:6px;">Quero fazer parte</h2>
      <p style="color:var(--muted);font-size:12px;margin-bottom:24px;">Preencha seus dados e diga como quer participar.</p>
      ${indicadorHtml}
      <form method="POST" action="/candidatura">
        ...
        <div class="field"><label>Profissão</label><input name="profissao" placeholder="Arquiteto, designer, artista..."></div>
        ...
        <div style="margin-bottom:24px;">
          <div style="font-size:11px;letter-spacing:.15em;text-transform:uppercase;color:var(--muted);margin-bottom:4px;">Como quer participar</div>
          <div style="font-size:11px;color:var(--muted);margin-bottom:14px;">Membro é a base — todos entram como membro. Marque também como deseja contribuir.</div>

          <!-- MEMBRO fixo -->
          <div class="funcao-card fixo" style="margin-bottom:8px;">
            <div class="check selecionado">✓</div>
            <div>
              <div class="funcao-nome">Membro</div>
              <div class="funcao-desc">Pertence ao Círculo. Base de todos os participantes.</div>
            </div>
          </div>

          <div style="display:flex;flex-direction:column;gap:8px;">${funcaoCards}</div>
        </div>
...
app.post('/candidatura', async (req, res) => {
  ...
  // sempre inclui membro
  if (!funcoes.includes('membro')) funcoes.unshift('membro');
  const funcoesStr = funcoes.join(', ');
  ...
  try {
    ...
    if (existe.rows.length) {
      return res.send(html('Solicitação', `<div class="container-sm">
        <div class="msg-erro">Este e-mail já tem uma solicitação ou cadastro no Círculo.</div>
        <a href="/login" class="btn btn-outline">Entrar no portal</a></div>`));
    }

    await pool.query(
      `INSERT INTO circulo_candidatos (nome, email, como_conheceu, convite_id, funcoes_desejadas)
       VALUES ($1, $2, $3, $4, $5)`,
      [nome, email, como_conheceu || null, convite_id || null, funcoesStr]
    );
    ...
    res.send(html('Solicitação enviada', `
      <div class="container-sm" style="text-align:center;padding-top:40px;">
        ...
        <p style="color:var(--muted);line-height:1.8;">Em breve você receberá um retorno.<br>O Círculo é pequeno por intenção.</p>
      </div>
    `));
  } catch (e) {
    console.error(e);
...
app.get('/cadastro/:token', async (req, res) => {
  try {
    ...
    res.send(html('Link inválido', `<div class="container-sm"><div class="msg-erro">Este link expirou ou é inválido.</div></div>`));
  }
});

app.post('/cadastro', async (req, res) => {
  const { token, senha, senha2 } = req.body;
  if (senha !== senha2) return res.send(html('Cadastro', `<div class="container-sm"><div class="msg-erro">As senhas não coincidem.</div><a href="javascript:history.back()" class="btn btn-outline">Voltar</a></div>`));
  try {
    const payload = jwt.verify(token, JWT_SECRET);
    const cand = await pool.query(`SELECT * FROM circulo_candidatos WHERE id = $1`, [payload.candidato_id]);
    ...
    const codigoConv = crypto.randomBytes(6).toString('hex');
    await pool.query(`INSERT INTO circulo_convites (membro_id, codigo) VALUES ($1,$2)`, [mid, codigoConv]);
    const codigoAquis = crypto.randomBytes(6).toString('hex');
    await pool.query(`INSERT INTO circulo_links_aquisicao (membro_id, codigo) VALUES ($1,$2)`, [mid, codigoAquis]);
    ...
    // salva funções desejadas
    if (c.funcoes_desejadas) {
      const slugs = c.funcoes_desejadas.split(',').map(s => s.trim()).filter(Boolean);
      for (const slug of slugs) {
        const fRow = await pool.query(`SELECT id FROM circulo_funcoes WHERE slug = $1`, [slug]);
        if (fRow.rows.length) {
          await pool.query(`INSERT INTO circulo_membro_funcoes (membro_id, funcao_id) VALUES ($1,$2) ON CONFLICT DO NOTHING`, [mid, fRow.rows[0].id]);
        }
      }
    }
    ...
    console.error(e);
    res.send(html('Erro', `<div class="container-sm"><div class="msg-erro">Erro: ${e.message}</div></div>`));
  }
});
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.redirect('/portal');
  } catch (e) { console.error(e); res.redirect('/login?erro=Erro+interno'); }
});
  ...
  try {
    ...
    res.send(html('Portal', `
      ...
      <div class="card" style="margin-bottom:24px;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:16px;">
          <div>
            ...
            <div style="font-size:11px;letter-spacing:.2em;text-transform:uppercase;color:var(--muted);margin-bottom:12px;">Membro desde ${dataEntrada} · ${m.codigo_membro || ''}</div>
      ...
      <div class="grid-3" style="margin-bottom:24px;">
        <div class="stat-box"><div class="num">${m.obras_que_encontraram_lar||0}</div><div class="label">Obras que encontraram lar</div></div>
        <div class="stat-box"><div class="num">${m.total_indicacoes||0}</div><div class="label">Pessoas indicadas</div></div>
        <div class="stat-box"><div class="num">${m.sugestoes_incorporadas||0}</div><div class="label">Sugestões incorporadas</div></div>
      </div>
      <div class="card">
      ...
      ${linkConvite ? `<div style="margin-top:24px;padding:16px;border:1px solid var(--border);border-radius:4px;">
    ...
    `, true));
  } catch (e) { console.error(e); res.send(html('Erro', `<div class="msg-erro">${e.message}</div>`, true)); }
});
...
app.get('/catalogo', authMembro, async (req, res) => {
  try {
    const obras = await pool.query(`
      ...
      GROUP BY o.id,o.nome,o.conceito,c.nome,o.tiragem_maxima ORDER BY o.id DESC`);
      ...
      <div class="card" style="margin-bottom:16px;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;">
          <div>
            ...
            <p style="font-size:13px;color:var(--muted);line-height:1.6;">${(o.conceito||'').substring(0,140)}...</p>
          </div>
          <div style="text-align:right;flex-shrink:0;">
            <div style="font-size:11px;color:var(--muted);">Tiragem</div>
    ...
    res.send(html('Catálogo', `
      ...
      <h2 style="font-size:28px;margin-bottom:32px;">Catálogo ALMARE</h2>
...
app.get('/meu-impacto', authMembro, async (req, res) => {
  try {
    const trans = await pool.query(`SELECT t.* FROM circulo_transacoes t WHERE t.membro_id=$1 ORDER BY t.criado_em DESC`, [req.membro.id]);
    ...
    const linhas = trans.rows.map(t => `<tr>
      <td>Obra #${t.obra_id}</td>
    ...
    res.send(html('Impacto', `
      ...
      <div class="grid-2" style="margin-bottom:32px;">
        <div class="stat-box"><div class="num">R$ ${parseFloat(s.saldo_disponivel).toFixed(2).replace('.',',')}</div><div class="label">Crédito disponível</div></div>
        <div class="stat-box"><div class="num">R$ ${parseFloat(s.saldo_total).toFixed(2).replace('.',',')}</div><div class="label">Total histórico</div></div>
      </div>
      <div class="card">
        <h3 style="font-size:18px;margin-bottom:20px;">Histórico</h3>
        ${trans.rows.length?`<table><thead><tr><th>Obra</th><th>Valor</th><th>Modalidade</th><th>Benefício</th><th>Status</th></tr></thead><tbody>${linhas}</tbody></table>`:'<p style="color:var(--muted);">Nenhuma venda ainda.</p>'}
      </div>
    `, true));
...
app.get('/sugestoes', authMembro, async (req, res) => {
  ...
  res.send(html('Voz', `
    ...
    <p style="color:var(--muted);margin-bottom:32px;">Sugira temas, formatos, ambientes. Anderson lê tudo.</p>
    <div class="card" style="margin-bottom:24px;">
      <form method="POST" action="/sugestoes">
        <div class="field"><label>Sua sugestão</label><textarea name="texto" required placeholder="Uma ideia de coleção, formato, tema..."></textarea></div>
        <button type="submit" class="btn btn-primary">Enviar</button>
      </form>
    </div>
    ${lista.rows.length?`<div class="card"><h3 style="font-size:16px;margin-bottom:16px;">Anteriores</h3>${itens}</div>`:''}
  `, true));
});
...
app.get('/meu-convite', authMembro, async (req, res) => {
  ...
  res.send(html('Convidar', `
    ...
    <p style="color:var(--muted);margin-bottom:32px;">Compartilhe com quem acredita que pertence ao Círculo.<br>Anderson aprova cada pessoa individualmente.</p>
    <div class="card">
      <div style="font-size:11px;letter-spacing:.2em;text-transform:uppercase;color:var(--muted);margin-bottom:12px;">Link pessoal</div>
      <div style="background:#0d0d0d;border:1px solid var(--border);border-radius:3px;padding:14px;font-size:13px;word-break:break-all;margin-bottom:16px;">${link}</div>
      <button onclick="navigator.clipboard.writeText('${link}');this.textContent='Copiado ✓'" class="btn btn-outline">Copiar link</button>
      <div style="margin-top:20px;font-size:12px;color:var(--muted);">${c?c.usos:0} pessoa(s) entrou pela sua indicação</div>
    </div>
  `, true));
});
...
// ADMIN
// ════════════════════════════════════════════════════════════════
app.get('/admin/login', (req, res) => res.send(html('Admin', `
  <div class="container-sm">
...
app.get('/admin', authAdmin, async (req, res) => {
  ...
  const linhaCand = candidatos.rows.map(c => `
    <tr>
      <td><strong>${c.nome}</strong><br><span style="font-size:11px;color:var(--muted)">${c.email}</span></td>
      <td style="color:var(--muted);font-size:12px;">${c.profissao||'—'}</td>
      <td style="font-size:12px;">
        ${(c.funcoes_desejadas||'membro').split(',').map(f=>`<span class="badge badge-gold" style="margin:2px;display:inline-block;">${f.trim()}</span>`).join('')}
      </td>
      <td style="color:var(--muted);font-size:12px;">${c.como_conheceu||'—'}</td>
  ...
  res.send(html('Admin', `
    ...
    <div class="card" style="margin-bottom:24px;">
      ...
      <table><thead><tr><th>Nome / E-mail</th><th>Profissão</th><th>Funções desejadas</th><th>Como conheceu</th><th>Data</th><th>Ação</th></tr></thead>
      <tbody>${linhaCand}</tbody></table>
    </div>`:''}
    <div class="card">
      <h3 style="font-size:18px;margin-bottom:20px;">Membros ativos</h3>
      <table><thead><tr><th>Nome</th><th>Código</th><th>E-mail</th><th>Crédito</th><th>Obras</th><th>Indicações</th></tr></thead>
      <tbody>${linhaMembros||'<tr><td colspan="6" style="color:var(--muted);text-align:center;padding:24px;">Nenhum membro ainda</td></tr>'}</tbody>
      </table>
    </div>
  ...
  try {
    ...
    await enviarEmail(c.email, 'Você foi convidado para o Círculo ALMARE', `
      <div style="background:#0a0a0a;color:#e8e8e8;padding:40px;font-family:Georgia,serif;">
        ...
        <p style="line-height:1.8;margin-bottom:24px;">Sua solicitação foi aprovada.</p>
        <a href="${link}" style="display:inline-block;background:#c9a96e;color:#000;padding:14px 32px;text-decoration:none;font-size:13px;letter-spacing:.15em;text-transform:uppercase;">Criar minha conta</a>
    ...
    // mostra link na tela para envio manual caso email não configurado
    res.send(html('Aprovado', `
      <div class="container-sm">
        <div class="msg-ok">Candidato aprovado.</div>
        <div class="card">
          <div style="font-size:13px;margin-bottom:16px;">Se o email não chegou, envie este link manualmente para <strong>${c.email}</strong>:</div>
          <div style="background:#0d0d0d;border:1px solid var(--border);border-radius:3px;padding:14px;font-size:12px;word-break:break-all;margin-bottom:16px;">${link}</div>
          <button onclick="navigator.clipboard.writeText('${link}');this.textContent='Copiado ✓'" class="btn btn-outline" style="margin-right:8px">Copiar link</button>
          <a href="/admin" class="btn btn-primary">Voltar ao painel</a>
        </div>
      </div>
    `));
...
app.get('/admin/sugestoes', authAdmin, async (req, res) => {
  const lista = await pool.query(`SELECT s.*, m.nome as membro_nome FROM circulo_sugestoes s JOIN circulo_membros m ON m.id=s.membro_id ORDER BY s.status ASC, s.criado_em DESC`);
  const itens = lista.rows.map(s => `
    <div style="padding:20px;border:1px solid var(--border);border-radius:4px;margin-bottom:12px;">
      ...
      <form method="POST" action="/admin/sugestoes/${s.id}/responder" style="display:flex;gap:8px;flex-wrap:wrap;">
        <input name="resposta" placeholder="Resposta (opcional)" value="${s.resposta||''}" style="flex:1;background:#0d0d0d;border:1px solid var(--border);color:var(--text);padding:8px 12px;border-radius:3px;font-size:13px;">
